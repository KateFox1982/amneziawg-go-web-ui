...
func (h *SocketHub) BroadcastServerStatus(serverID, status string) {
	h.io.Emit("server_status", map[string]interface{}{
		"server_id": serverID,
		"status":    status,
	}) //nolint:errcheck
}
...
func (h *SocketHub) registerConnectionHandlers() {
	h.io.On("connection", func(clients ...interface{}) {
		...
		client.Emit("status", map[string]interface{}{ //nolint:errcheck
...
func (h *SocketHub) buildTrafficData() map[string]interface{} {
	servers := h.mgr.copyServers()

	clientTraffic := map[string]interface{}{}
